...
async function inviteReader(){
  ...
  const {error}=result;actionLoading.value=false
...
<template>
  ...
  <div v-else class="app-shell">
    ...
    <main><header class="topbar"><button class="menu-button" @click="mobileNav=true"><Menu/></button><div><p class="eyebrow">{{isAdmin?'ADMINISTRATOR PORTAL':'AUTHOR WORKSPACE'}}</p><h1>{{pageTitle}}</h1></div><div class="top-actions"><button class="icon-button"><Bell :size="19"/></button><button class="primary" @click="showNewBook=true"><Plus :size="18"/> New book</button></div></header>
      ...
      <section v-else class="content"><div class="welcome-row"><div><h2>{{isAdmin?'Administrator settings':'Workspace settings'}}</h2><p>{{isAdmin?'Manage author access and platform security.':'Your security controls apply to every reading session.'}}</p></div></div><div class="settings-grid"><section class="panel settings-card"><h3>Manuscript protection</h3><div class="setting-confirm"><Check/> Author and reader watermark</div><div class="setting-confirm"><Check/> Copying and printing blocked</div><div class="setting-confirm"><Check/> Confidentiality agreement required</div></section><section class="panel settings-card"><h3>Owner account</h3><p>{{OWNER_EMAIL}}</p><p class="muted-copy">New accounts receive access only through an author or reader invitation.</p></section><section v-if="profile?.role==='admin'" class="panel settings-card author-access-card"><h3>Author access</h3><p>Invite or remove authors individually. Removed authors immediately lose manuscript-management access.</p><form class="inline-invite" @submit.prevent="inviteAuthor"><input v-model="newAuthorEmail" type="email" placeholder="author@example.com" aria-label="Author email"/><button class="primary" :disabled="actionLoading">Invite author</button></form><div v-if="authorInvitations.length" class="author-invite-list"><div v-for="invite in authorInvitations" :key="invite.id"><span><strong>{{invite.email}}</strong><small>{{invite.status}}</small></span><button v-if="!invite.revoked_at" class="danger-link" @click="revokeAuthor(invite)">Remove author</button></div></div><p v-else class="muted-copy">No other authors have been invited.</p></section></div></section>
    </main>
  </div>
