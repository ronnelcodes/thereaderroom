...
const showSignup = ref(false), showLoginChoice = ref(false), showSignupPassword = ref(false), signupMessage = ref(''), signupError = ref('')
const signupForm = ref({full_name:'',email:'',role:'author',password:'',confirm_password:''})
...
const showNewBook = ref(false), showChapterReview = ref(false), showInvite = ref(false), showReader = ref(false), showAgreement = ref(false), showComment = ref(false), showReaderComments = ref(false), showNotice = ref(false)
...
const signupPasswordRules = computed(()=>({length:signupForm.value.password.length>=10,capital:/[A-Z]/.test(signupForm.value.password),lowercase:/[a-z]/.test(signupForm.value.password),number:/\d/.test(signupForm.value.password),special:/[^A-Za-z0-9]/.test(signupForm.value.password)}))
const signupIsValid = computed(()=>signupForm.value.full_name.trim()&&/^\S+@\S+\.\S+$/.test(signupForm.value.email.trim())&&['author','reader'].includes(signupForm.value.role)&&Object.values(signupPasswordRules.value).every(Boolean)&&signupForm.value.password===signupForm.value.confirm_password)
...
const selectedBookComments = computed(()=>feedback.value.filter(comment=>comment.book_id===selectedBook.value?.id))
const chapterComments = computed(()=>selectedBookComments.value.filter(comment=>comment.chapter_id===selectedChapter.value?.id))

function flash(message){toast.value=message;setTimeout(()=>toast.value='',2800)}
function formatUploadDate(value){return value?new Date(value).toLocaleString(undefined,{dateStyle:'medium',timeStyle:'short'}):'Upload time unavailable'}
...
function startSignup(role='author'){
  signupForm.value={full_name:'',email:'',role:['author','reader'].includes(role)?role:'author',password:'',confirm_password:''}
  signupError.value='';signupMessage.value='';showSignupPassword.value=false;showSignup.value=true
}
async function createPublicAccount(){
  signupError.value=''
  if(!signupIsValid.value){signupError.value=signupForm.value.password!==signupForm.value.confirm_password?'The passwords do not match.':'Complete every field and meet all password requirements.';return}
  actionLoading.value=true
  try{
    const account={...signupForm.value,email:signupForm.value.email.trim().toLowerCase(),full_name:signupForm.value.full_name.trim()}
    const {data,error}=await supabase.auth.signUp({email:account.email,password:account.password,options:{emailRedirectTo:`${window.location.origin}/${account.role}-login`,data:{full_name:account.full_name,signup_role:account.role,public_signup:true}}});if(error)throw error
    showSignup.value=false;portal.value=account.role;window.history.replaceState({},'',`/${account.role}-login`)
    if(data.session){session.value=data.session;await loadWorkspace();flash('Your account is ready. Welcome to The Reader Room.')}
    else{portal.value='choice';window.history.replaceState({},'','/');signupMessage.value=`Check ${account.email} to confirm your account, then sign in as a${account.role==='author'?'n author':' reader'}.`}
  }catch(error){signupError.value=error.message||'Your account could not be created.'}
...
async function loadReaderData(){
  const [ir,fr]=await Promise.all([
    supabase.from('reader_invitations').select('*,books(*)').order('invited_at',{ascending:false}),
    supabase.from('comments').select('*,chapters(title,chapter_number),books(title)').eq('reader_id',session.value.user.id).order('created_at',{ascending:false})
  ]);if(ir.error)throw ir.error;if(fr.error)throw fr.error
  invitations.value=ir.data||[];books.value=invitations.value.map(i=>i.books).filter(Boolean);feedback.value=fr.data||[]
}
...
async function removeBook(book){
  if(!isAdmin.value)return flash('Administrator access is required to remove a book.')
  const confirmed=window.confirm(`Permanently remove “${book.title}”?\n\nThis deletes every manuscript version, chapter, reader assignment, reading record, and feedback item connected to this book. This cannot be undone.`)
  if(!confirmed)return
  actionLoading.value=true
  try{
    const {error}=await supabase.from('books').delete().eq('id',book.id);if(error)throw error
    let storageCleanupFailed=false
    const folder=`${book.author_id}/${book.id}`
    const listed=await supabase.storage.from('manuscripts').list(folder,{limit:1000})
    if(listed.error)storageCleanupFailed=true
    else if(listed.data?.length){
      const paths=listed.data.map(file=>`${folder}/${file.name}`)
      const removed=await supabase.storage.from('manuscripts').remove(paths);if(removed.error)storageCleanupFailed=true
    }
    if(selectedBook.value?.id===book.id){selectedBook.value=null;selectedChapter.value=null;chapters.value=[];showReader.value=false}
    await loadAuthorData()
    flash(storageCleanupFailed?'Book removed. Private file cleanup needs administrator attention.':`${book.title} was permanently removed.`)
  }catch(error){flash(error.message||'The book could not be removed.')}finally{actionLoading.value=false}
}
...
async function submitComment(){
  ...
  const {data,error}=await supabase.from('comments').insert({book_id:selectedBook.value.id,chapter_id:selectedChapter.value.id,reader_id:session.value.user.id,sentence_index:selectedSentenceIndex.value,quoted_text:selectedSentence.value,comment_text:commentText.value.trim()}).select('*,chapters(title,chapter_number),books(title)').single();actionLoading.value=false
  if(error)flash(error.message);else{feedback.value.unshift(data);showComment.value=false;flash('Comment saved. You can see it in Your comments.')}
}
...
<template>
  ...
  <main v-else-if="!session&&portal==='choice'" class="public-home">
    <header class="public-nav"><a class="public-logo" href="/" aria-label="The Reader Room home"><img src="/assets/reader-room-primary-logo.svg?v=2" alt="The Reader Room"/></a><nav><a href="#for-authors">For authors</a><a href="#for-readers">For readers</a><a href="#how-it-works">How it works</a></nav><div><button class="public-signin" @click="showLoginChoice=true">Sign in</button><button class="primary" @click="startSignup('author')">Create an account</button></div></header>
    <div v-if="signupMessage" class="signup-success"><Check :size="18"/><span>{{signupMessage}}</span><button @click="signupMessage=''" aria-label="Dismiss"><X :size="16"/></button></div>
    <section class="public-hero"><div class="public-hero-copy"><p class="eyebrow">PRIVATE BETA READING, BEAUTIFULLY ORGANIZED</p><h1>Better feedback begins in a better room.</h1><p>Share works in progress with trusted readers, collect thoughtful comments in context, and keep every revision moving forward.</p><div class="public-hero-actions"><button class="primary public-cta" @click="startSignup('author')">Join as an author <ChevronRight :size="18"/></button><button class="secondary public-cta" @click="startSignup('reader')">Join as a reader</button></div><small><ShieldCheck :size="15"/> Private manuscripts, role-based access, and personal reader watermarks</small></div><div class="public-hero-preview" aria-label="Reader feedback preview"><div class="preview-top"><span><BookOpen :size="16"/> The Last Garden</span><small>CHAPTER 4 OF 18</small></div><div class="preview-page"><p class="preview-chapter">CHAPTER FOUR</p><h2>What the River Kept</h2><p>When Mara reached the old bridge, the morning fog had already folded the town into silence.</p><mark>The letter in her pocket felt heavier with every step.</mark><div class="preview-comment"><span>JD</span><div><strong>Jordan commented</strong><p>This detail creates great tension. I want to know what is in the letter.</p></div></div><p>Below her, the river carried the first leaves of autumn toward a place no one could name.</p></div></div></section>
    <section class="public-trust"><span>Built for stories before they are ready for the world</span><div><span><LockKeyhole/> Protected access</span><span><MessageSquare/> Feedback in context</span><span><Gauge/> Reading progress</span></div></section>
    <section id="for-authors" class="public-feature"><div class="feature-art author-art"><div class="mini-project"><p class="eyebrow">MANUSCRIPT OVERVIEW</p><h3>The Last Garden</h3><div><span>12</span><small>chapters</small><span>6</span><small>readers</small><span>34</span><small>comments</small></div><i><b style="width:68%"></b></i></div><div class="floating-note"><Sparkles/><span><strong>All feedback, one place</strong><small>Review notes by book and chapter.</small></span></div></div><div class="feature-copy"><p class="eyebrow">FOR AUTHORS</p><h2>Spend less time chasing notes and more time shaping your story.</h2><p>Upload your manuscript, review its chapter structure, and invite readers into a private workspace. Feedback stays attached to the exact sentence and manuscript version it belongs to.</p><ul><li><Check/> Review and edit chapters before publishing</li><li><Check/> Save revisions without losing earlier feedback</li><li><Check/> See reader activity and comments together</li></ul><button class="text-button public-learn" @click="startSignup('author')">Start your author workspace <ChevronRight/></button></div></section>
    <section id="for-readers" class="public-feature reverse"><div class="feature-art reader-art"><div class="reader-note-stack"><article><blockquote>“The porch light flickered twice.”</blockquote><p>This feels important. Is it a signal?</p><small>Chapter 2 · just now</small></article><article><blockquote>“She finally opened the blue door.”</blockquote><p>I have been waiting for this moment.</p><small>Chapter 7 · yesterday</small></article></div></div><div class="feature-copy"><p class="eyebrow">FOR READERS</p><h2>Read privately. Respond thoughtfully. Remember every note.</h2><p>Your reading shelf keeps invited manuscripts in one place. Comment directly on a sentence, track chapter progress, and revisit everything you have shared with the author.</p><ul><li><Check/> Comment without leaving the manuscript</li><li><Check/> See your comments by book and chapter</li><li><Check/> Keep every author’s work confidential</li></ul><button class="text-button public-learn" @click="startSignup('reader')">Create your reader account <ChevronRight/></button></div></section>
    <section id="how-it-works" class="public-steps"><p class="eyebrow">HOW THE ROOM WORKS</p><h2>From first draft to useful feedback in three steps.</h2><div><article><span>01</span><BookOpen/><h3>Bring in the story</h3><p>Authors upload a manuscript and make sure every chapter is ready for readers.</p></article><article><span>02</span><Users/><h3>Open the right doors</h3><p>Invite trusted readers and give each person private access to the right book.</p></article><article><span>03</span><MessageSquare/><h3>Turn reading into insight</h3><p>Readers leave notes in context while authors organize feedback for the next revision.</p></article></div></section>
    <section class="public-final"><div><p class="eyebrow">YOUR STORY DESERVES THOUGHTFUL FIRST READERS</p><h2>Make room for better feedback.</h2><p>Choose the role that fits you today. You can start an author workspace or create a reader account in a few minutes.</p><div><button class="primary public-cta" @click="startSignup('author')">Sign up as an author</button><button class="secondary public-cta" @click="startSignup('reader')">Sign up as a reader</button></div></div></section>
    <footer class="public-footer"><img src="/assets/reader-room-logo-white.svg?v=3" alt="The Reader Room"/><p>A protected place for stories, trusted readers, and feedback that helps the work grow.</p><div><button @click="selectPortal('author')">Author sign in</button><button @click="selectPortal('reader')">Reader sign in</button><button @click="selectPortal('admin')">Administrator sign in</button></div><small>© {{new Date().getFullYear()}} The Reader Room by Vibe &amp; Vision Collective</small></footer>
  </main>
  ...
  <div v-if="showSignup" class="modal-backdrop highest signup-backdrop" @click.self="showSignup=false"><section class="modal signup-modal"><button class="modal-close" @click="showSignup=false"><X/></button><p class="eyebrow">CREATE YOUR ACCOUNT</p><h2>Choose your place in the room.</h2><p>Sign up as an author to share work, or as a reader to join private beta reads.</p><div class="signup-role-grid"><button type="button" :class="{active:signupForm.role==='author'}" @click="signupForm.role='author'"><BookOpen/><span><strong>Author</strong><small>Upload manuscripts and organize feedback</small></span></button><button type="button" :class="{active:signupForm.role==='reader'}" @click="signupForm.role='reader'"><Users/><span><strong>Reader</strong><small>Read invited books and leave comments</small></span></button></div><label>Full name<input v-model="signupForm.full_name" autocomplete="name" placeholder="Your full name"/></label><label>Email address<input v-model="signupForm.email" type="email" autocomplete="email" placeholder="you@example.com"/></label><label>Password<div class="password-input-wrap"><input v-model="signupForm.password" :type="showSignupPassword?'text':'password'" autocomplete="new-password" placeholder="At least 10 characters"/><button type="button" :aria-label="showSignupPassword?'Hide password':'Show password'" @click="showSignupPassword=!showSignupPassword"><EyeOff v-if="showSignupPassword"/><Eye v-else/></button></div></label><ul class="password-rules"><li :class="{met:signupPasswordRules.length}"><Check/>10+ characters</li><li :class="{met:signupPasswordRules.capital}"><Check/>Uppercase letter</li><li :class="{met:signupPasswordRules.lowercase}"><Check/>Lowercase letter</li><li :class="{met:signupPasswordRules.number}"><Check/>Number</li><li :class="{met:signupPasswordRules.special}"><Check/>Symbol</li></ul><label>Confirm password<input v-model="signupForm.confirm_password" type="password" autocomplete="new-password" placeholder="Type it again" @keyup.enter="createPublicAccount"/></label><p v-if="signupForm.confirm_password&&signupForm.password!==signupForm.confirm_password" class="password-mismatch">Passwords do not match.</p><p v-if="signupError" class="form-error">{{signupError}}</p><button class="primary full" :disabled="actionLoading||!signupIsValid" @click="createPublicAccount">{{actionLoading?'Creating account…':`Create ${signupForm.role} account`}}</button><small class="signup-terms">By creating an account, you agree to respect manuscript confidentiality and the creative work shared here.</small></section></div>
  ...
  <div v-if="showReader" class="reader-overlay"><header class="reader-header"><button class="reader-back" @click="showReader=false;showReaderComments=false"><ChevronLeft :size="18"/> Exit reader</button><div><strong>{{selectedBook?.title}}</strong><small>{{selectedChapter?.title||'No chapters'}}</small></div><div class="reader-tools"><button v-if="!isAuthor" class="reader-comments-button" @click="showReaderComments=true"><MessageSquare :size="15"/> Your comments <span>{{selectedBookComments.length}}</span></button><span><LockKeyhole :size="15"/> Protected session</span></div></header><div class="reader-progress"><i :style="{width:chapters.length?`${((chapters.findIndex(c=>c.id===selectedChapter?.id)+1)/chapters.length)*100}%`:'0%'}"></i></div><main class="manuscript-wrap"><aside class="chapter-rail"><p class="eyebrow">CONTENTS</p><button v-for="chapter in chapters" :key="chapter.id" :class="{active:chapter.id===selectedChapter?.id}" @click="selectedChapter=chapter"><span>{{chapter.chapter_number}}</span><div><strong>{{chapter.title}}</strong></div></button></aside><article class="manuscript"><div class="watermarks"><span v-for="n in 12" :key="n">{{selectedBook?.author_name?.toUpperCase()}} · {{readerEmail}}</span></div><template v-if="selectedChapter"><p class="chapter-number">CHAPTER {{selectedChapter.chapter_number}}</p><h1>{{selectedChapter.title}}</h1><p v-if="!isAuthor" class="reader-instruction"><MessageSquare :size="15"/> Click any sentence to comment.</p><div class="prose"><button v-for="(sentence,index) in chapterSentences" :key="index" @click="openSentence(sentence,index)">{{sentence}} </button></div><div v-if="!isAuthor" class="chapter-end"><Check/><h3>Reached the end?</h3><button class="primary" @click="completeChapter">Complete chapter</button></div></template><div v-else class="empty-state"><FileText/><h2>No chapters found</h2></div></article><aside v-if="!isAuthor" class="comment-rail reader-comment-history"><div class="comment-head"><strong>Your comments</strong><span>{{chapterComments.length}}</span></div><small>On this chapter</small><article v-for="comment in chapterComments" :key="comment.id"><blockquote>“{{comment.quoted_text}}”</blockquote><p>{{comment.comment_text}}</p><small>{{formatUploadDate(comment.created_at)}}</small></article><div v-if="!chapterComments.length" class="comment-empty"><MessageSquare/><p>Your comments on this chapter will appear here.</p></div><button class="text-button" @click="showReaderComments=true">See all book comments <ChevronRight :size="14"/></button></aside></main><div v-if="showNotice" class="protection-notice"><ShieldCheck/><div><strong>This manuscript is protected.</strong><span>Copying, printing, and screenshots are not permitted.</span></div></div></div>
  ...
  <div v-if="showReaderComments" class="modal-backdrop highest" @click.self="showReaderComments=false"><section class="modal reader-comments-modal"><button class="modal-close" @click="showReaderComments=false"><X/></button><p class="eyebrow">YOUR COMMENT HISTORY</p><h2>{{selectedBook?.title}}</h2><p>Everything you have commented on in this manuscript.</p><div v-if="selectedBookComments.length" class="reader-comments-list"><article v-for="comment in selectedBookComments" :key="comment.id"><div><strong>{{comment.chapters?.title||'Chapter'}}</strong><small>{{formatUploadDate(comment.created_at)}}</small></div><blockquote>“{{comment.quoted_text}}”</blockquote><p>{{comment.comment_text}}</p></article></div><div v-else class="empty-state compact"><MessageSquare/><h3>No comments yet</h3><p>Click a sentence in the manuscript to leave your first note.</p></div></section></div>
  <div v-if="showLoginChoice" class="modal-backdrop highest" @click.self="showLoginChoice=false"><section class="modal compact login-choice-modal"><button class="modal-close" @click="showLoginChoice=false"><X/></button><p class="eyebrow">WELCOME BACK</p><h2>Which workspace are you opening?</h2><div class="signup-role-grid"><button type="button" @click="showLoginChoice=false;selectPortal('author')"><BookOpen/><span><strong>Author sign in</strong><small>Manage manuscripts and feedback</small></span></button><button type="button" @click="showLoginChoice=false;selectPortal('reader')"><Users/><span><strong>Reader sign in</strong><small>Open your private reading shelf</small></span></button></div><button class="text-button admin-login-link" @click="showLoginChoice=false;selectPortal('admin')"><ShieldCheck :size="15"/> Administrator sign in</button></section></div>
