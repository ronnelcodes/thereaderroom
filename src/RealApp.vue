...
const newBook = ref({ title:'', author_name:'Teejaye Deloach', genre:'', deadline:'', manuscriptText:'', files:[] })
...
const nav = computed(()=>[
  ...
  ...(isAdmin.value?[{id:'authors',label:'Authors',icon:Users}]:[]),
...
function resetAuthForm(){authPassword.value='';authCode.value='';codeSent.value=false;authMode.value='password';authError.value=''}
...
async function checkPortalEmail(email){
  ...
  if(gate.error||!gate.data)throw new Error(portal.value==='admin'?'This email is not approved for administrator access.':portal.value==='author'?'This email has not been invited as an author.':'No active reader invitation was found for this email.')
}
...
async function sendCode(){
  ...
  try{
    ...
    const {error}=await supabase.auth.signInWithOtp({email,options:{shouldCreateUser:true,data:{full_name:email.split('@')[0]}}});if(error)throw error
    codeSent.value=true
...
async function signOut(){passwordSetupRequired.value=false;sessionStorage.removeItem('readerRoomPasswordSetup');await supabase.auth.signOut();profile.value=null;books.value=[];invitations.value=[];feedback.value=[]}
...
async function createBook(){
  ...
  try{
    ...
    for(const [index,file] of newBook.value.files.entries()){
      const path=`${session.value.user.id}/${book.id}/v1-${String(index+1).padStart(3,'0')}-${Date.now()}-${file.name.replace(/[^a-zA-Z0-9._-]/g,'-')}`
      const up=await supabase.storage.from('manuscripts').upload(path,file);if(up.error)throw up.error;paths.push(path)
    }
    const fileLabel=newBook.value.files.length>1?`${newBook.value.files.length} chapter files`:newBook.value.files[0]?.name||null
    if(paths.length)await supabase.from('books').update({manuscript_path:paths[0],manuscript_filename:fileLabel}).eq('id',book.id)
    await supabase.from('manuscript_versions').insert({book_id:book.id,version_number:1,storage_path:paths[0]||null,filename:fileLabel,created_by:session.value.user.id})
    let chapterRows
    ...
    const rows=chapterRows.map(c=>({...c,book_id:book.id,version_number:1}));const cr=await supabase.from('chapters').insert(rows);if(cr.error)throw cr.error
    await supabase.from('reminder_settings').insert({book_id:book.id})
    newBook.value={title:'',author_name:profile.value.full_name||'Teejaye Deloach',genre:'',deadline:'',manuscriptText:'',files:[]};showNewBook.value=false;await loadAuthorData();flash(`${book.title} is ready for readers.`)
...
async function removeReader(i){
  if(!window.confirm(`Remove ${i.email} from this manuscript? Their access and reading progress for this invitation will be removed.`))return
  const {error}=await supabase.from('reader_invitations').delete().eq('id',i.id)
  if(error)flash(error.message);else{await loadAuthorData();flash('Reader removed. The reader spot is available now.')}
}
...
<template>
  ...
  <main v-else-if="!session" class="auth-page">
    ...
    <section class="auth-card">
      ...
      <div v-else-if="authMode==='password'">
        <button class="text-button auth-back" @click="portalHome">← Choose another entrance</button><p class="eyebrow">{{portal==='admin'?'ADMINISTRATOR PORTAL':portal==='author'?'AUTHOR PORTAL':'READER PORTAL'}}</p><h2>{{portal==='admin'?'Manage The Reader Room.':portal==='author'?'Manage your work.':'Open your reading shelf.'}}</h2><p>Sign in with the email address approved for this portal and your secure password.</p><label>Email address<input v-model="authEmail" type="email" autocomplete="email" placeholder="you@example.com"/></label><label>Password<input v-model="authPassword" type="password" autocomplete="current-password" placeholder="Your password" @keyup.enter="signInWithPassword"/></label><button class="primary full" :disabled="actionLoading" @click="signInWithPassword">{{actionLoading?'Signing in…':'Sign in securely'}} <ChevronRight :size="17"/></button><button class="text-button setup-password-link" @click="setupPassword">First sign-in or forgot your password?</button>
      </div>
  ...
  <main v-else-if="passwordSetupRequired" class="auth-page password-setup-page">
    <section class="auth-brand"><img class="auth-primary-logo" src="/assets/reader-room-logo-white.svg?v=3" alt="The Reader Room"/><p>Your invitation is verified. Create the password you will use for future visits.</p><div class="auth-promise"><LockKeyhole/><span>Secure account access<small>Your verification code cannot be used as your password</small></span></div></section>
    <section class="auth-card password-card"><img class="auth-card-logo" src="/assets/reader-room-primary-logo.svg?v=2" alt="The Reader Room"/><p class="eyebrow">SECURE YOUR ACCOUNT</p><h2>Create your password.</h2><p>Use a unique password that you do not use on another website.</p><label>New password<input v-model="newPassword" type="password" autocomplete="new-password" placeholder="At least 10 characters"/></label><ul class="password-rules"><li :class="{met:passwordRules.length}"><Check/> At least 10 characters</li><li :class="{met:passwordRules.capital}"><Check/> One capital letter</li><li :class="{met:passwordRules.lowercase}"><Check/> One lowercase letter</li><li :class="{met:passwordRules.number}"><Check/> One number</li><li :class="{met:passwordRules.special}"><Check/> One special character</li></ul><label>Confirm password<input v-model="confirmPassword" type="password" autocomplete="new-password" placeholder="Enter the same password" @keyup.enter="savePassword"/></label><p v-if="confirmPassword&&newPassword!==confirmPassword" class="password-mismatch">The passwords do not match.</p><button class="primary full" :disabled="actionLoading||!passwordIsValid" @click="savePassword">{{actionLoading?'Saving password…':'Save password and enter'}}</button><p v-if="authError" class="form-error">{{authError}}</p><button class="text-button cancel-password" @click="signOut">Cancel and sign out</button></section>
  </main>
  ...
  <div v-else class="app-shell">
    ...
    <main><header class="topbar"><button class="menu-button" @click="mobileNav=true"><Menu/></button><div><p class="eyebrow">{{isAdmin?'ADMINISTRATOR PORTAL':'AUTHOR WORKSPACE'}}</p><h1>{{pageTitle}}</h1></div><div class="top-actions"><button class="icon-button"><Bell :size="19"/></button><button class="primary" @click="showNewBook=true"><Plus :size="18"/> New book</button></div></header>
      ...
      <section v-else class="content"><div class="welcome-row"><div><h2>{{isAdmin?'Administrator settings':'Workspace settings'}}</h2><p>{{isAdmin?'Platform security and owner information.':'Your security controls apply to every reading session.'}}</p></div></div><div class="settings-grid"><section class="panel settings-card"><h3>Manuscript protection</h3><div class="setting-confirm"><Check/> Author and reader watermark</div><div class="setting-confirm"><Check/> Copying and printing blocked</div><div class="setting-confirm"><Check/> Confidentiality agreement required</div></section><section class="panel settings-card"><h3>Owner account</h3><p>{{OWNER_EMAIL}}</p><p class="muted-copy">New accounts receive access only through an author or reader invitation.</p></section></div></section>
    </main>
  </div>
