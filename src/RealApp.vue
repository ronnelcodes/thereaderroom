...
function selectPortal(kind){portal.value=kind;authEmail.value='';authCode.value='';codeSent.value=false;authError.value='';window.history.replaceState({},'',`/${kind}-login`)}
...
async function sendCode(){
  ...
  if(gate.error||!gate.data){actionLoading.value=false;authError.value=portal.value==='admin'?'This email is not approved for administrator access.':portal.value==='author'?'This email has not been invited as an author.':'No active reader invitation was found for this email.';return}
  ...
  <main v-else-if="!session" class="auth-page">
    ...
    <section class="auth-card">
      ...
      <div v-else-if="!codeSent">
        <button class="text-button auth-back" @click="portalHome">← Choose another entrance</button><p class="eyebrow">{{portal==='admin'?'ADMINISTRATOR PORTAL':portal==='author'?'AUTHOR PORTAL':'READER PORTAL'}}</p><h2>{{portal==='admin'?'Manage The Reader Room.':portal==='author'?'Manage your work.':'Open your reading shelf.'}}</h2><p>Enter the exact email address approved for this portal. We’ll send a one-time verification code.</p><label>Email address<input v-model="authEmail" type="email" autocomplete="email" placeholder="you@example.com" @keyup.enter="sendCode"/></label><button class="primary full" :disabled="actionLoading" @click="sendCode">{{actionLoading?'Checking…':'Email my code'}} <ChevronRight :size="17"/></button>
      </div>
      <div v-else>
