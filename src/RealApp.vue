...
async function saveReviewedBook(){
  ...
  }catch(error){flash(error.message?.includes('save_book_revision')?'Run supabase/add-book-editing.sql in Supabase, then try again.':error.message||'The book could not be saved.')}finally{actionLoading.value=false}
}
async function saveInitialBook(){
  const metadata=reviewBook.value
  const {data:book,error}=await supabase.from('books').insert({author_id:session.value.user.id,title:metadata.title.trim(),author_name:metadata.author_name.trim(),genre:metadata.genre,description:metadata.description?.trim()||null,overall_deadline:metadata.overall_deadline||null,status:'active'}).select().single();if(error)throw error
  const paths=[]
  ...
  if(paths.length){const update=await supabase.from('books').update({manuscript_path:paths[0],manuscript_filename:fileLabel}).eq('id',book.id);if(update.error)throw update.error}
  const version=await supabase.from('manuscript_versions').insert({book_id:book.id,version_number:1,storage_path:paths[0]||null,filename:fileLabel,created_by:session.value.user.id});if(version.error)throw version.error
  const rows=reviewChapters.value.map((chapter,index)=>({book_id:book.id,version_number:1,chapter_number:index+1,title:chapter.title.trim(),content:chapter.content.trim()}));const chaptersInsert=await supabase.from('chapters').insert(rows);if(chaptersInsert.error)throw chaptersInsert.error
  const reminders=await supabase.from('reminder_settings').insert({book_id:book.id});if(reminders.error)throw reminders.error
  const title=book.title;closeChapterReview();resetNewBook();await loadAuthorData();flash(`${title} is ready for readers.`)
}
