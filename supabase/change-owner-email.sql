-- Run this once after add-author-invitations.sql.
-- The Reader Room owner account is thereaderroom@teejayedeloach.com.
...
begin
  ...
  where lower(email) = lower(target_email)
    and lower(email) <> 'thereaderroom@teejayedeloach.com';
end;
$$;
...
where lower(email) in (
  'teejayedeloachwrites@gmail.com',
  'teejayedeloach@teejayedeloach.com'
);

update public.profiles
set role = 'admin'::public.user_role
where lower(email) = 'thereaderroom@teejayedeloach.com';

grant execute on function public.can_use_author_portal(text) to anon, authenticated;
grant execute on function public.revoke_author(uuid) to authenticated;
