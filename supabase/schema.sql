...
select id, lower(email), coalesce(raw_user_meta_data ->> 'full_name', split_part(email, '@', 1)),
       case when lower(email) = 'thereaderroom@teejayedeloach.com' then 'admin'::public.user_role when raw_user_meta_data ->> 'signup_role' = 'author' then 'author'::public.user_role else 'reader'::public.user_role end,
       coalesce(raw_user_meta_data ->> 'public_signup' = 'true', false)
from auth.users
on conflict (id) do update set
  email = excluded.email,
...
begin
  if auth.uid() is null then raise exception 'Authentication required'; end if;
  if not public.can_manage_book(p_book_id) then raise exception 'You are not authorized to edit this book'; end if;
  if nullif(trim(p_title), '') is null or nullif(trim(p_author_name), '') is null then raise exception 'Book title and author name are required'; end if;
  if nullif(trim(p_genre), '') is null then raise exception 'Genre is required'; end if;
  if p_status not in ('draft', 'active', 'complete', 'archived') then raise exception 'Invalid book status'; end if;
  if p_chapters is null or jsonb_typeof(p_chapters) <> 'array' or jsonb_array_length(p_chapters) = 0 then raise exception 'At least one chapter is required'; end if;
  ...
  insert into public.manuscript_versions (book_id, version_number, storage_path, filename, created_by)
  values (p_book_id, next_version, existing_path, existing_filename, auth.uid());
  insert into public.chapters (book_id, version_number, chapter_number, title, content)
  select p_book_id, next_version, chapter.ordinality::integer,
         trim(chapter.value ->> 'title'), trim(chapter.value ->> 'content')
  from jsonb_array_elements(p_chapters) with ordinality as chapter(value, ordinality);
  update public.books
  set title = trim(p_title), author_name = trim(p_author_name), genre = trim(p_genre),
      description = nullif(trim(p_description), ''), overall_deadline = p_overall_deadline,
      status = p_status::public.book_status, current_version = next_version
  where id = p_book_id;
  return next_version;
end;
$$;
