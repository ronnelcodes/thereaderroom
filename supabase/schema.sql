...
as $$
  select exists(
    select 1 from public.books b where b.id = target_book and b.author_id = auth.uid()
  ) or exists(
...
create policy books_author_update on public.books for update to authenticated using (author_id = auth.uid() or public.is_admin()) with check (author_id = auth.uid() or public.is_admin());
create policy books_author_delete on public.books for delete to authenticated using (author_id = auth.uid() or public.is_admin());

create policy versions_read on public.manuscript_versions for select to authenticated using (public.can_read_book(book_id));
create policy versions_author_all on public.manuscript_versions for all to authenticated using (exists(select 1 from public.books b where b.id = book_id and (b.author_id = auth.uid() or public.is_admin()))) with check (exists(select 1 from public.books b where b.id = book_id and (b.author_id = auth.uid() or public.is_admin())));

create policy chapters_read on public.chapters for select to authenticated using (public.can_read_book(book_id));
create policy chapters_author_all on public.chapters for all to authenticated using (exists(select 1 from public.books b where b.id = book_id and (b.author_id = auth.uid() or public.is_admin()))) with check (exists(select 1 from public.books b where b.id = book_id and (b.author_id = auth.uid() or public.is_admin())));

create policy invitations_author_all on public.reader_invitations for all to authenticated using (exists(select 1 from public.books b where b.id = book_id and (b.author_id = auth.uid() or public.is_admin()))) with check (exists(select 1 from public.books b where b.id = book_id and (b.author_id = auth.uid() or public.is_admin())));
...
create policy agreements_read on public.reader_agreements for select to authenticated using (reader_id = auth.uid() or exists(select 1 from public.reader_invitations ri join public.books b on b.id = ri.book_id where ri.id = invitation_id and (b.author_id = auth.uid() or public.is_admin())));
...
create policy progress_read on public.reading_progress for select to authenticated using (reader_id = auth.uid() or exists(select 1 from public.reader_invitations ri join public.books b on b.id = ri.book_id where ri.id = invitation_id and (b.author_id = auth.uid() or public.is_admin())));
...
create policy comments_read on public.comments for select to authenticated using (reader_id = auth.uid() or exists(select 1 from public.books b where b.id = book_id and (b.author_id = auth.uid() or public.is_admin())));
...
create policy comments_author_update on public.comments for update to authenticated using (exists(select 1 from public.books b where b.id = book_id and (b.author_id = auth.uid() or public.is_admin()))) with check (exists(select 1 from public.books b where b.id = book_id and (b.author_id = auth.uid() or public.is_admin())));
...
create policy questionnaires_author_all on public.questionnaires for all to authenticated using (exists(select 1 from public.books b where b.id = book_id and (b.author_id = auth.uid() or public.is_admin()))) with check (exists(select 1 from public.books b where b.id = book_id and (b.author_id = auth.uid() or public.is_admin())));
...
create policy questions_author_all on public.questionnaire_questions for all to authenticated using (exists(select 1 from public.questionnaires q join public.books b on b.id = q.book_id where q.id = questionnaire_id and (b.author_id = auth.uid() or public.is_admin()))) with check (exists(select 1 from public.questionnaires q join public.books b on b.id = q.book_id where q.id = questionnaire_id and (b.author_id = auth.uid() or public.is_admin())));
create policy responses_read on public.questionnaire_responses for select to authenticated using (reader_id = auth.uid() or exists(select 1 from public.questionnaire_questions qq join public.questionnaires q on q.id = qq.questionnaire_id join public.books b on b.id = q.book_id where qq.id = question_id and (b.author_id = auth.uid() or public.is_admin())));
...
create policy reminders_author_all on public.reminder_settings for all to authenticated using (exists(select 1 from public.books b where b.id = book_id and (b.author_id = auth.uid() or public.is_admin()))) with check (exists(select 1 from public.books b where b.id = book_id and (b.author_id = auth.uid() or public.is_admin())));
...
using (bucket_id = 'manuscripts' and ((storage.foldername(name))[1] = auth.uid()::text or public.is_admin()));
...
returns trigger language plpgsql as $$
begin
  if (select count(*) from public.reader_invitations where book_id = new.book_id and revoked_at is null) >= 10 then
...
create trigger reader_limit_before_insert before insert on public.reader_invitations
