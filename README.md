...
- Administrator-only permanent book removal with related reader records and feedback cleanup
- Original book upload date and time displayed in the dashboard and Books page
...
- Reader comment history by book and chapter
- Saved chapter progress
- Full-height manuscript scrolling at standard browser zoom
...
- Public author and reader sign-up with email confirmation
- Original public marketing homepage for authors and beta readers
...
To enable public author and reader registration on an existing installation, run `supabase/add-public-signup.sql` once in the Supabase SQL Editor. The migration is idempotent and safely limits self-selected roles to author or reader. It never permits public administrator registration.
...
The public homepage lets a visitor register as an author or reader. A new reader starts with an empty shelf until an author or administrator grants book access. Existing administrator tools can still create an author or reader without sending an invitation, choose the role, set or generate a temporary password, and optionally assign a reader to one book. The user must replace an administrator-created temporary password after first sign-in. Email invitations remain available.

In Supabase Authentication settings, enable email and password sign-ups. Keep email confirmation enabled, set the Site URL to the deployed site, and add the site root plus `/author-login` and `/reader-login` to the allowed redirect URLs. Public accounts choose only the author or reader role. Administrator access remains restricted to the owner account.
